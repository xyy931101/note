...
3. 自增型的主键可以减小page的碎片，提升空间和内存的使用。如果使用非自增主键（如果是学号或者身份证作为主键），由于每次插入数据的主键都是认为定义的（随机的），因此每次插入数据可能插入到当前数据页的中间的某一个位置，此时Mysql不得不为了将新记录插入到合适的位置而移动数据，这将频繁的移动，分页造成大量的碎片，得到不紧凑的索引结构，大大增加了开销。
