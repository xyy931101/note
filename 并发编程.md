...
​	处理器并不能完成所有操作，至少要与内存进读取运算数据、存储计算结果、这个I/O操作是很难消除的。又由于计算机的存储设备与CPU的计算速度存在着几个数量级的差异，所以现代计算机不得不加入一层或多层读写速度尽可能接近于CPU运算速度的高速缓存来作为内存与处理器之间的缓冲，让运算能快速运行。但由于现代计算机基本都是多核处理器，每个处理器都有自己的高速缓存，从而引入了**缓存一致性问题**

## volatile关键字

### volatile的读写内存语义

- 写语义：当写一个volatlie变量时，JMM会把该线程对应的本地内存中的共享变量写回主内存
- 读语义：当读一个volatile变量事，JMM会把该线程对应的本地内存置为无效，线程将从主内存中获取共享变量

### volatile的实现细节

有violatile修饰的属性,在编译成汇编指令后会多出一个**LOCK**前缀,LOCK前缀的指令在多核处理器下会引发一下两件事情

1. 将当前处理器缓存行的数据写回到系统内存
2. 这个写回内存的操作会使其他CPU里面缓存了该内存地址无效，处理器通过总线嗅探机制，检查自己的缓存值是不是过期了
...
- [ ] 在每个volatile的写操作之前插入一个StoreStore屏障
- [ ] 在每个volatile的写操作之后插入一个StoreLoad屏障
- [ ] 在每个volatile的读操作之后插入一个LoadLoad屏障
- [ ] 在每个volatile的读操作之后插入一个LoadStore屏障

![volatile禁止指令重排](image/volatile禁止指令重排.jpeg)

#### 不保证其原子性

```
	volatile 很对于任意单个volatile的变量的读/写具有原子性，但是类似于volatile++这种复合操作不具有原子性
```
        ...
        if (instance == null){
            synchronized (Full.class){
               if (instance == null){
                 	 ...
                 	 //cotorInstance(memory);2.初始化对象
                 	 //instance = memory		 3.设置instance指向刚分配的内存地址
                 	 //如果不加volatile的话,2 3步骤可能会进行重排序
                   instance = new Full();
               }
            }
        }
...
- cacheLine：当从内存中取单元到cache中时，会一次取一个cacheline大小的内存区域到cache中，然后存进相应的cacheline中(默认大小63KB)

- L1、L2缓存成为本地核心内缓存，即一个核一个。如果你的机器是4核，那就是有4个L1+4个L2

- L3缓存是所有核共享的。即不管你的CPU是几核，这个CPU中只有一个L3

  ![image-20210221134906634](image\image-20210221134906634.png)
...
​	synchronized的4种锁状态，级别由低到高依次为：**无锁状态、匿名偏向状态：没有线程拿锁。**、**偏向锁状态**、**轻量级锁状态**、**重量级锁状态**

​	对象头部记录了锁的标志位,及偏向线程ID

- ​	**无锁状态(匿名偏向状态)**   也就是对象刚创建出来,没有任何线程进行抢占
...
- ​    **轻量级锁**      有除了锁对象头信息记录的线程之外的线程对锁进行抢占,  这时候会把**偏向锁信息给撤销**，然后各线程开始CAS自旋抢占锁
...
1. 字节码层面
   monitorenter  -》代码块 -》  monitorexit   monitorexit（**有两个monitorexit,因为内部做了try catch**）

2. JVM层面

   当线程获取锁时，会把改线程对本地内存置为无效，从而使得被监视器(monitor)保护的代码临界区必须从主内存中读取共享变量
...
1. **新建(NEW)**：新创建了一个线程对象 初始化线程，拷贝当前线程的inheritableThreadLocals变量

2. **运行(RUNNABLE)**：Java线程中将就绪（**ready**）和运行中（**running**）两种状态笼统的成为“运行”。
   线程对象创建后，其他线程(比如main线程）调用了该对象的start()方法。该状态的线程位于可运行线程池中，等待被线程调度选中，获取cpu 的使用权，此时处于就绪状态（ready）。就绪状态的线程在获得cpu 时间片后变为运行中状态（running）

   **Thread.start()先用JNI进入JVM层，创建对应的JAVAThread，再由JavaThread创建操作系统线程，然后利用JavaCalls进入Java层，让新线程执行Runnable.run()。**

3. **阻塞(BLOCKED)**：阻塞状态是指线程因为某种原因放弃了cpu 使用权，也即让出了cpu timeslice，暂时停止运行。直到线程进入可运行(runnable)状态，才有机会再次获得cpu timeslice 转到运行(running)状态。与**WAITING**的区别是，在等待获取一个排他锁

4. **等待(WAITING)**：处于这种状态的线程不会被处理器分配执行时间，需要等待其他线程显示唤醒。以下方法会出现
   
   - 没有设置timeout参数的Object:wait()
   - 没有设置timeout参数的Thread::join()
   - LockSupport::park()
   
5. **超时等待(TIME_WAITING)**：该状态不同于WAITING，在指定时间内由系统自动唤醒。以下方法会出现
   - Thread.::sleep()
...
![image-20210221142146403](image/769239fa3ecbb1ce4d8d9e1e5ce52830.jpeg)
...
![image-20210221165040495](image\image-20210221165040495.png)CONFLICT (content): Merge conflict in

#### 配置参数标准

- 核心线程数   
  - CPU密集型（CPU-bound） 线程数 = CPU核数+1 (现代CPU支持超线程)
...
>>>>>>> 1e88d75ffd19a17e9c2e68a23b69b26969301680
...
InheritableThreadLocal是ThreadLocal的子类，它就是用来解决父线程向子线程传递线程私有数据问题的
...
InheritableThreadLocal则是给当前线程的inheritableThreadLocals变量赋值
 其次，在创建线程实例，即new Thread()时，父线程使用自己的inheritableThreadLocals通过调用createInheritedMap方法来构造子线程的ThreadLocalMap并赋值给子线程的
...
#### **局限性：父线程无法通过InheritableThreadLocal向池化的子线程(线程池)传递线程私有数据**

#### 解决方案：使用transmittable-thread-local(alibaba)

[https://www.jianshu.com/p/2b0c0ff40182](https://www.jianshu.com/p/2b0c0ff40182)
