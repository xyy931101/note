...
2. JVM层面
   ...
   volatile写的内存语义：在写volatile变量时，**JMM会把改线程对本地内存的共享变量刷新到主内存**
   
   volatile读的内存语义：在读volatile变量时，**JMM会把该线程的本地内存置为无效，从主内存中读取**
   
   volatile内存区的读写**都加屏障**，从而实现禁止指令重排
...
```
	volatile 很适合只有一个线程修改，其他线程读取的情况。volatile 变量被修改之后，对其他线程立即可见。由于java的运算操作符并非原子操作，所以volatile不是线程安全的
```
...
![image-20210221142146403](image\image-20210221142146403.png)
